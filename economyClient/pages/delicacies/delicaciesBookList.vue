		...
		<view class="contain">
			<mescroll-body @down="downCallBack" @init ="initMescroll" @up="upCallback" top="130">
				<view class="ht-card card"  v-for="(item,index) in dellicaciesList" :key="index">
					<view class="card-bd card-bd-box-shadow" @click="toDetail(item.shopId)">
						<view class="ct-list">
							...
							<view class="righttxt">
								...
								<view class="coupon coupon2" v-if="item.reserveId != null">
									<view class=""><label style="align-items: center;" class="mark red">订</label> </view>
								</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			roomReserve: function(shopId) {
				uni.navigateTo({
					url: "/pages/food/foodReserve?shopId=" + shopId
				})
			},
			toDetail(shopId){
				uni.navigateTo({
