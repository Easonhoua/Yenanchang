		 ...
		 <mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption" @scroll="scroll" @topclick="topClick">
			...
			<view class="cityStory">
				...
				<scroll-view  scroll-x="true">
					<view class="oul">
						<view class="li" v-for="(item,index) in chengshiList" :key="index" @click="toActivityDetail(item.activityId,item.activityChildType)">
							<image class="topimg" :src="item.imagePath[0]"></image>
							<view class="bottomtxt">{{ item.activityTitle }}</view>
						</view>
					</view>
				</scroll-view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//进入城事详情
			toActivityDetail(activityId,activityChildType) {
				let statusStr = 0;
