<template>
	<view class="page">
				...
				<view class="couponbox">
					<view class="couponbox-item" v-for="(item,index) in quanlist" :key='index'>
						...
						<view class="couponbox-item-con">
							...
							<text class="gz">{{ item.orderAmount }}</text>
		...
		<view class="mask" v-show="ishowModel"  @touchmove.stop.prevent= "" @click.stop="hidemodel">
			<view class="model" @click.stop="">
				<image class="scusseimg" :src=" isright==true?'/static/img_new/activity/right.png':'/static/img_new/activity/right.png' "></image>
				<view class="title">{{ modeltitle }}</view>
				<view class="modelbtn" @click="hidemodel">继续投票</view>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			queryDetail(activityId) {
				...
				that.request.get(url, data).then(res => {
					...
					console.log(e)
					// that.ishowModel = true
					// that.isright = false
					// that.modeltitle = "领取失败"
				})
			},
