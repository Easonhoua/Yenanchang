<template>
	<view class="page play strategy">
		<view class="top-banner" @click="enlargeImage(0)">
			<video class="video" v-if="guideInfo.videoPath" :poster="guideInfo.thumbnailPath[0]" :src="guideInfo.videoPath"></video>
			<image class="img" v-else-if="guideInfo.thumbnailPath && guideInfo.thumbnailPath.length > 0" :src="guideInfo.thumbnailPath[0]" mode="aspectFill"></image>
		</view>
		<view class="userinfor">
			...
			<jyf-parser :html="guideInfo.guideDetail ? '<style>p{width:100% !important;}#top{max-width: 100%;} .img{width:100% !important;} img{width: 100%}</style>'+guideInfo.guideDetail : ''" @linkpress="golook"></jyf-parser>
	...
	methods: {
		...
		readDetail() {
			...
			this.request.get(url, data).then(res => {
				...
				if(this.guideInfo.videoPath){
					this.guideInfo.videoPath = this.util.formatImagePath(this.guideInfo.videoPath)
				}
				// 是否已经收藏
