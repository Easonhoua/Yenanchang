<template>
	<view class="content-white">
		...
		<scroll-view v-if="markers.length" class="list" scroll-y="true" >
			<view class="list-item-view" v-for="(item,index) in markers" :key="index" @click="swiperListItemClick(item)">
				<view class="list-item-leftView">
					<view class="list-item-name map-line-limit">{{item.tradeName?item.tradeName:'--'}}</view>
					<view class="list-item-address map-line-limit">{{item.tradeTheme?item.tradeTheme.replace(/<\/?[^>]*>/g, "").replace(/\&(nbsp;)/ig, ""):'--'}}</view>
				</view>
		 ...
		 <!-- <swiper v-if="markers.length" class="list" :vertical="true" :current="swiperCurrent" @change="swiperChange">
		 	<swiper-item style="height: 220rpx;" v-for="(item,index) in markers" :key="index">
	...
	export default {
		...
		methods: {
			...
			getAllMapData:function(){
				...
				this.allList = this.shopList.concat(this.hotelList).concat(this.showList).concat(this.freeList).concat(this.studyList).concat(this.sportList).concat(this.scenicList).concat(this.foodList).concat(this.cultureList);
...
<style>
	...
	.list-item-view {
		...
		border-radius: 8rpx;
		box-shadow: 0rpx 0rpx 12rpx 2rpx rgba(70, 145, 123, 0.4);
		background-color: #FFFFFF;
	...
	.menu-select-btn {
		...
		color: #42917A;
		/* border-radius: 8rpx; */
		border-bottom: 6rpx solid #42917A;
	}
	.map-line-limit{
		display: -webkit-box;
