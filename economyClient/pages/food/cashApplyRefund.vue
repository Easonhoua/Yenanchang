		...
		<view class="order-cellbox">
			<view class="order-cell-top">
				...
				<view class="order-cell-bd">
					...
					<view class="tip">
						<text>单价：</text><text class="txt">￥{{cashData.discountPercent}}</text>
	...
	export default {
		...
		methods:{
				...
				that.request.post(url, data).then(res => {
					if(res.returncode == 0){
						...
						if(res.data && res.data.itemList){
							that.queryCashData(res.data.itemList[0].productId); //查询套餐信息
						}
					}
				})
			},
			...
			reduce:function(){
				...
				this.orderTotalAmount = this.cashData.discountAmount * this.orderRefundNum;
			...
			refundOrder:function(){
				let url = '/memberapi/api/cash/refundOrder.do';
				let data={
				...
				that.request.post(url, data).then(res => {
					if(res.returncode == 0){
						that.request.toastTips("申请成功");
						 setTimeout(function() {
						 	uni.redirectTo({
								url:'/pages/cash/orderView?orderId='+this.orderData.orderId
						 	});
						 }, 1000);
					}else{
