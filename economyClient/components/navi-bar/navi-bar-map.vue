<template>
	<view class="">
		<view v-if="headFlag" class="header" style="background-color: #FFFFFF;">
			<image class="icon" src="/static/img_new/scenic/h_fh_b.png" @click="back"></image>
			<view class="right-icon">
				<image class="icon" v-if="showFavoriteButton==true"  :src="theIsFavorite?'/static/img_new/scenic/h_ysc.png':'/static/img_new/scenic/h_sc_b.png'"  @click="favoriteWithData"> </image>
				<image v-if="showShareButton" class="icon" src="/static/img_new/scenic/h_fx_b.png" @click="shareButtonClick"></image>
				<image class="icon" src="/static/img_new/scenic/h_gd_b.png"></image>
			</view>
		</view>
		<view v-else class="header">
			<image class="icon" src="/static/img_new/scenic/h_fh.png" @click="back"></image>
			<view class="right-icon">
				<image class="icon" v-if="showFavoriteButton==true"  :src="theIsFavorite?'/static/img_new/scenic/h_ysc.png':'/static/img_new/scenic/h_sc.png'"  @click="favoriteWithData"> </image>
				<image v-if="showShareButton" class="icon" src="/static/img_new/scenic/h_fx.png" @click="shareButtonClick"></image>
...
<script>
	export default {
		...
		props: {
			...
			// 收藏类型(1:店铺、2:评价、3:攻略、4:资讯、5:精彩)，有收藏功能的话必填
			favoriteType: {
				type: String,
				default: "1"
			},
		...
		methods: {
			...
			shareItemClick: function(item) {
				...
				if(this.isStrategy){
					this.shareData.url = this.request.getBaseUrl() + "/wxapp/" + route + "?guideId=" + this.relationId;
				}else{
