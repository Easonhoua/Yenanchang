		...
		<view class="order-cellbox">
			<view class="order-cell-top">
				...
				<view class="order-cell-bd">
					...
					<view class="tip">
						<text>单价：</text><text class="txt" v-if="activityData.activityCost != 0">￥{{activityData.activityCost}}</text>
						<text class="txt" v-else>免费</text>
						<!-- <text>购买数量：</text><text class="txt">1</text> -->
	...
	export default {
		...
		methods: {
			...
			refundOrder:function(){
				let url = '/memberapi/api/activityReserve/refundOrder.do';
				let data ={
					orderId : this.orderData.orderId,
					applicant : this.applicant,
					reason : this.reason,
					imagePath : this.imagePath,
					orderRefundNum : 1,	 //活动只有一张
				}
				var that = this;
				that.request.post(url, data).then(res => {
					if(res.returncode == 0){
						that.request.toastTips("申请成功");
						 setTimeout(function() {
						 	uni.redirectTo({
								url:'/pages/activity/orderView?orderId='+this.orderData.orderId
						 	});
						 }, 1000);
					}else{
