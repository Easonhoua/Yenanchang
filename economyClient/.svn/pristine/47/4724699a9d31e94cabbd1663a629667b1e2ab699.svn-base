			...
			<view>
				...
				<view class="info-view">
					<view class="display-row">
						...
						<image v-if="shopsData.shopsPhone" class="shop-location-image" src="/static/img/common/phone.png" @click="toPhone(shopsData.shopsPhone)"></image>
					</view>
				...
				<sportActivityList ref='sportActivityList' :shopId="shopId"></sportActivityList>
...
<script>
	...
	import sportActivityList from "@/pages/activity/sportActivityList.vue"
	export default {
			...
			queryDetail: function(mescroll,shopId) {
				...
				this.request.get(url, data).then(res => {
					...
					if(that.shopsData.details){
						that.shopsData.details =that.shopsData.details.replace(/\<img/g,'<img style="width:100%;height:auto;display:block"').replace(/\width: (\d*)px/g,'').replace(/\height: (\d*)px/g,'');
					}
