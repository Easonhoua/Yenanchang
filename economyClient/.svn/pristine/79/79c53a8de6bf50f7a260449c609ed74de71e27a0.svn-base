<template>
	<view class="page placelist">
		...
		<view class="tab">
			...
			<view class="item" :class="tabIndex==0 ? 'active' : ''" @click="ontabtap(0)">推荐</view>
			<view class="item" :class="tabIndex==1 ? 'active' : ''" @click="ontabtap(1)">最热</view>
			<view class="item" :class="tabIndex==2 ? 'active' : ''" @click="ontabtap(2)">最近</view>
			<view class="item" :class="tabIndex==3 ? 'active' : ''" @click="ontabtap(3)">最新</view>
		</view>
				...
				<view class="module">
					<view class="list play">
						<view class="item" v-for="(item,index) in nearShopList" :key="index" @click="toShopDetail(item)">
							<view class="imgbox">
								<image class="img" :src="item.thumbnailPath" mode="scaleToFill"></image>
							</view>
							<view class="txt">
	...
	export default {
		...
		methods:{
			...
			ontabtap(index) {
				this.tabIndex = index;
				if(this.tabIndex==1){
					this.sortFlag="sequence-asc pageview-desc "; //浏览量
