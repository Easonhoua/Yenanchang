<template>
	<view class="conten">
		...
		<mescroll-body @down="dowmCallBack" @up="upCallback" @init="initMescroll" :down="downOption" >
	...
	export default {
		...
		methods: {
			...
			getUserIndexs: function() {
				...
				this.lowIndexs = [
					{
						"url":"https://www.ncnews.com.cn/nctt/more.html",
			...
			listCellClick: function(item) {
				if (item.text == '更多应用'){
					this.util.gotoWebView(item.url,"更多应用");
