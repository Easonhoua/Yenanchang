<template>
	<view class="layout letime">
		<screenComponent platformTypeId="7" @screenLike="screenLike" ref="myScreen"></screenComponent>
		<view class="contain">
			<mescroll-body @down="downCallBack" @init ="initMescroll" @up="upCallback" top="130">
				<view class="ht-card card" v-for="(item,index) in scenicList" :key="index">
					<view class="card-bd card-bd-box-shadow">
		...
		methods: {
			...
			upCallback(mescroll)
			{
				...
				this.request.get(url,sendDate,mescroll).then(res=>{
					...
					that.scenicList = that.scenicList.concat(currListData); //追加新数据
				})
			},
